...
const NodeMailer = () => {
  ...
  const sendEmail = async (e) => {
    ...
    if (!emailLetters) {
      toast.error("You need to type/paste your letter(s)");
      return;
    }

    try {
      const toastId = toast.loading(
        `Sending letter to ${numberCount === 1 ? `${emailCount.toLocaleString()} email` : `${emailCount.toLocaleString()} emails`}`
      );
      ...
      const res = await request.post("/send", newEmail);
      ...
      if (
        ...
        toast.error("Your SMTP is currently not sending emails.", {
          duration: 90000,
        });
      }
                ...
                <div className="relative w-full h-full mt-14">
                  <div className="flex flex-row items-stretch justify-end flex-shrink-0 w-auto space-x-3 space-y-0 sm:w-auto md:w-auto sm:flex-row md:flex-row sm:space-y-0 md:space-y-0 sm:items-center md:items-center sm:space-x-3 md:space-x-3">
                    <button
                      type="button"
                      onClick={getSMS}
                      className="inline-flex items-center mt-4 px-5 py-2.5 text-sm text-center text-green-800 font-semibold bg-green-200 rounded-lg hover:bg-green-800 hover:text-green-200"
                    >
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        width="24"
                  ...
                  <div className="relative p-6 mt-4 bg-white border-t-2 border-blue-700 rounded-lg shadow sm:p-5">
                    ...
                    <form className="mt-8">
                      ...
                      <div>
                        ...
                        <p className="pt-8 text-sm text-gray-600">Attachments are automatically added into rotation. You can upload up to 10 different attachments</p>
